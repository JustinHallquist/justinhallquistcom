export default function TreeNav({ tree, activeNode, ignore, handleSetActivePath }) {
  ...
  const rootNodeEl = (node, active, path, depth) => <div path={path} onClick={() => handleSetActivePath(path)} key={`${node}.${depth}`} className={`block font-medium hover:underline ${active ? 'dark:text-blue-500' : 'dark:text-gray-300'}`}  >{node}</div>
  const nodeEl = (node, active, parentActive, path, depth) => {
    const tabs = Array(depth * 2).fill(0).reduce((memo, _) => `${memo}-`, '');
    return <div path={path} onClick={() => handleSetActivePath(path)} key={`${node}.${depth}`} className={`block font-medium hover:underline ${active ? 'dark:text-blue-500' : 'dark:text-gray-300'} ${!parentActive ? 'hidden' : ''}`}  ><span>{tabs} </span>{node}</div>
  }
